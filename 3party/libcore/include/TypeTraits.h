...
template <>
struct is_ushort<unsigned short>
{
    enum
    {
        value = 1
...
template <>
struct is_ulonglong<unsigned long long>
{
    enum
    {
        value = 1
...
template <>
struct is_integer<const unsigned short>
{
    enum
    {
        value = 1
...
template <>
struct is_unsigned<const long long>
{
    enum
    {
        value = 0
...
struct is_number
{
    static const bool value = is_integer<T>::value || is_float<T>::value;
};
...
struct is_const_pointer
{
    ...
    enum
    {
        value = (sizeof(true_t) == sizeof(is_const_pointer_tester((void (*)(T))0)))
    };
};
...
struct is_pointer
{
    ...
    enum
    {
        value = (sizeof(true_t) == sizeof(is_pointer_tester((T)0)))
    };
};
...
{
    template <typename U>
    static true_t is_class_or_union_tester(void (U::*)());
    template <typename U>
    static false_t is_class_or_union_tester(...);

    enum
    {
        value = (sizeof(true_t) == sizeof(is_class_or_union_tester<T>(0)))
    };
};
...
struct is_same_type
{
    ...
    template <typename X>
    struct is_same_type_tester<X, X>
    {
        enum
        {
            value = 1
